  ...
  def meta_loss(self,
    ...
    def update(net, fx, x, state):
      ...
      with tf.name_scope("gradients"):
       ...
       def gradients(fx, x, grad, city_x_y ):

          gradnext = np.zeros(grad.shape)

          for i, path, cityarray in zip(range(len(city_x_y[0])), x, city_x_y[0]):
            # print("cityarray  fx===")
            # print(cityarray)
            # print("cityarray  i===")
            # print(i)
            ...
            pathbest = np.zeros((itermax, len(path)))


            pathbest[0] = path

            calculate = yu.yuqun(coord = cityarray, numantall = (len(path) - 2) )
            ...
            while iter < itermax:
              ...
              # print("path  fx===")
              # print(pathbest[iter-1])
              # print("lengthbest  fx===")
              # print(lengthbest[iter - 1])

            deltasx = pathbest[iter-1] - path
            deltaslength = lengthbest[iter-1] - fx

            for j in range(len(path)):
                if deltasx[j] != 0 :
                    gradnext[i][j] =  deltaslength/ deltasx[j]
                else:
                    gradnext[i][j] = 1e8
          ...
          return gradnext.astype('Float32')  # will modify in the future

       with tf.variable_scope('hidden4'):
          gradientslist = list()
          for i in x:
              # Trainable variable.
              gradx = tf.get_variable(
                "grad",
                shape=[x[0].get_shape()[0], x[0].get_shape()[1]],
                dtype=tf.float32,
                initializer=tf.random_uniform_initializer(minval=0, maxval=4000))
              tt = tf.py_func(gradients, [fx, i, gradx, tf.contrib.framework.get_variables_by_name("city_x_y")], [tf.float32])
              # grad = tf.Print(grad, [tt], "x=", None, 127*799)
              gradx = tf.assign(gradx, tf.to_float(tt[0]))
              gradx = tf.Print(gradx, [gradx], "gradx=", None, 127 * 799)
              gradientslist.append(gradx)
       ...
       getdeltas = tf.py_func(next_deltas,[deltas,x, fx ],[tf.double], stateful= False)
       deltasx = tf.get_variable(
            "deltasx",
            shape=[x[0].get_shape()[0], x[0].get_shape()[1]],
            dtype=tf.float32,
            initializer=tf.random_uniform_initializer(minval=0, maxval=4000))
       deltasx= tf.assign(deltasx, tf.to_float(getdeltas[0][0]))
       deltas = (deltasx,)
      ...
      with tf.name_scope("dx"):
        for subset, key, s_i in zip(subsets, net_keys, state):
          ...
          for idx, j in enumerate(subset):
            x_next[j] += deltas[idx]
          tf.Print(x_next,  [x_next],  "x_next==============",None,128*10)
